...
public class Car {
	...
	//			필드변수(멤버변수) : 필드에 선언한 변수(속성), 값을 할당하지 않으면 JVM의 기본값을 할당한다.
	
	//				인스턴스변수	:   필드변수 중 none static 변수 
	//										인스턴스마다 개별적인 값을 지니는 변수
	//				클래스변수		:   필드변수 중 static 변수
	//										클래스로 만든 모든 인스턴스가 공유하는 변수
	//										만약 클래스변수를 변경하게 되면 클래스로 만들어진 모든 인스턴스가 영향을 받는다.
	
	
	//		지역 : 메서드 내부, 메서드가 시작될 때 메모리에 올라가며 메서드가 종료 될 때 메모리에서 내려온다.
	
	//			   매개변수 : 외부의 값을 전달 받기 위해 메서드 선언부에 작성되는 변수
	//			   지역변수 : 메서드 내부에서 선언된 변수, 반드시 초기화를 해야지만 사용할 수 있다. 
	//						
